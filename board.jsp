...
<jsp:include page="header.jsp" />
	<%
		...
		try {
			...
			result = query.executeQuery(Board.SelectRowCount("board"));
			result.next();
		...
		try {
			result = query.executeQuery(Board.SelectPage("board", pageNumber));
			while (result.next()) {
	...
	<table class="table table-hover">
		...
		<%
			for (int i = 0; i < SORT_COUNT && i < boardList.size(); i++) {
				...
				out.print("<td>");
				out.print("<a href=\"boardOpen.jsp?&pageNumber=" + pageNumber + "&index=" + board.GetIndex() + "\">");
		...
		{
			out.print("<a href=\"board.jsp?pageNumber=" + i + "\">");
			out.print("[" + i + "]");
			out.print("</a>");
		}
...
<p align="right">
	<a href="boardWrite.jsp?&boardName=board"><button type="button" class="btn btn-primary">글쓰기</button></a>
	</p>
	</div>
